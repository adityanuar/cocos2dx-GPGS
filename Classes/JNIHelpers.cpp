...
long JniHelpers::jniCommonLongCall(const char* methodName, const char* classPath) {
#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
	cocos2d::JniMethodInfo minfo;

	bool isHave = cocos2d::JniHelper::getStaticMethodInfo(minfo, classPath, methodName, "()J");

	if (isHave)
	{

		jlong ret = minfo.env->CallStaticLongMethod(minfo.classID, minfo.methodID);
