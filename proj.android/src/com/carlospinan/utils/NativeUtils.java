...
public class NativeUtils {
	...
	/*
	 * Google Analytic Tracking set screen and set event
	 */
	public static void setScreenName(final String name) {
		app.runOnUiThread(new Runnable() {

			@Override
			public void run() {
				app.setScreenName(name);
			}
		});
	}
	
	public static void setEvent(final String categoryId, final String actionId, final String labelId) {
		app.runOnUiThread(new Runnable() {

			@Override
			public void run() {
				app.setEvent(categoryId, actionId, labelId);
				Log.d("Analytic", "sET eVENT");
			}
		});
	}
	...
	/*
	 * Get the score of leaderboard
	 */
	static long mPoints = 0;
	public static void loadScoreOfLeaderBoard(final String leaderboardID) {
		app.runOnUiThread(new Runnable() {
			...
			public void run() {
				if (isSignedIn())
				{
					Games.Leaderboards.loadCurrentPlayerLeaderboardScore(app.getCustomApiClient(), leaderboardID, LeaderboardVariant.TIME_SPAN_ALL_TIME, LeaderboardVariant.COLLECTION_PUBLIC).setResultCallback(new ResultCallback<Leaderboards.LoadPlayerScoreResult>() {
				        @Override
				        public void onResult(final Leaderboards.LoadPlayerScoreResult scoreResult) {
				            if (isScoreResultValid(scoreResult)) {
				            	NativeUtils.mPoints = scoreResult.getScore().getRawScore();
				            }
				        }
				    });
				}
					...
					String message = context.getResources().getString(
							R.string.fail_getscore_leaderboard);
					message = message.replace("{leaderboardID}", leaderboardID);
					displayAlert(message);
				}
			}
	...
	public static long getRawScore()
	{
		return NativeUtils.mPoints;
	}
